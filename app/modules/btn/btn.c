...
static int _btn_config(btn_gpio *btn) {
  if (!gpio_is_ready_dt(&btn->spec)) {
    printk("%s was not ready during init.", btn->name);
		return -EIO;
	} else if (0 > gpio_pin_configure_dt(&btn->spec, GPIO_INPUT)) {
    printk("%s could not be configured during init.", btn->name);
		return -EIO;
  } else if (0 > gpio_pin_interrupt_configure_dt(&btn->spec, GPIO_INT_EDGE_TO_ACTIVE)) {
    printk("%s interrupt could not be configured during init.", btn->name);
		return -EIO;
  } else {
...
int BTN_init() {
  ...
  for (uint8_t i = 0; i < NUM_BTNS; i++) {
    ...
    if (rv < 0) {
      printk("Failed to configure %s", _btns[i]->name);
      return rv;
    }
  }
...
bool BTN_is_pressed(btn_id btn) {
  if (btn >= NUM_BTNS || btn < 0) {
    printk("Couldn't check the state of an invalid button");
    return false;
