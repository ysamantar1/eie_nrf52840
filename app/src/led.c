...
static const led_gpio _led_1 = {.spec=GPIO_DT_SPEC_GET(LED1_NODE, gpios), .name="LED0"};
static const led_gpio _led_2 = {.spec=GPIO_DT_SPEC_GET(LED2_NODE, gpios), .name="LED0"};
static const led_gpio _led_3 = {.spec=GPIO_DT_SPEC_GET(LED3_NODE, gpios), .name="LED0"};
 ...
 */
int _config_led(const led_gpio *led) {
...
int init_leds() {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    if (rv < 0) {
      DEBUG("Failed to configure %s", _leds[i].name);
      return rv;
    }
  }
  return rv;
...
int toggle_led(led_inst led_instance) {
  if (led_instance >= NUM_LEDS) {
    DEBUG("Failed to toggle invalid LED: LED%d", led_instance);
    return -EINVAL;
  } else {
    int rv = gpio_pin_toggle_dt(&_leds[led_instance].spec);
    if (rv) {
      DEBUG("Failed to toggle %s", _leds[led_instance].name);
    }
    return rv;
  }
}